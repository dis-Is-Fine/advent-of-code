...
void assignType(Card* card){
    ...
    int jokerCount = 0;
    for(int i = 0; i < 5; i++){
        if(card->numbers[i] == CARD_J) {jokerCount++; continue;}
    ...
    int largest = largestInIntArray(instances[1], numberCount)+jokerCount;
    /* There is only one number repeated 5 times */
    if(instances[1][0] == 5 || largest == 5){card->type = TYPE_FIVE_OF_KIND; return;}
